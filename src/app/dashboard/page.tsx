...
import { Plus, Home, Calendar, Image as ImageIcon, Search, Lock } from 'lucide-react';
  ...
  function renderGrids() {
    ...
    const EventCard = ({ event }: { event: Event }) => (
      <div 
        ...
        className="group relative flex flex-col bg-slate-900 border border-white/10 rounded-2xl overflow-hidden hover:border-white/20 transition-all duration-300 cursor-pointer shadow-xl hover:shadow-indigo-500/10 hover:-translate-y-1 h-[320px]"
      >
        <div className={`h-32 w-full bg-gradient-to-br ${getGradient(event._id)} relative overflow-hidden shrink-0`}>
          ...
          <div className="absolute top-4 right-4 flex items-center gap-2">
            {event.isPrivate && (
              <div className="bg-black/40 backdrop-blur-md border border-white/10 px-3 py-1 rounded-full flex items-center gap-1.5 shadow-lg">
                <Lock size={12} className="text-pink-400" />
                <span className="text-xs font-medium text-white/90">Private</span>
              </div>
            )}
            <div className="bg-black/40 backdrop-blur-md border border-white/10 px-3 py-1 rounded-full flex items-center gap-1.5 shadow-lg">
    ...
    const MobileCreateCard = () => (
      <div 
        ...
        className="sm:hidden flex flex-col items-center justify-center h-[320px] rounded-2xl border-2 border-dashed border-white/10 bg-white/[0.02] hover:bg-white/[0.05] hover:border-indigo-500/50 transition-all cursor-pointer group"
      >
    ...
    return (
      <div className="space-y-12">
        {myEvents.length > 0 && (
          <div>
            <h2 className="text-xl font-bold text-white mb-6 border-b border-white/10 pb-3">Your Events</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              <MobileCreateCard />
        ...
        {allEventsToDisplay.length > 0 && (
          <div>
            <h2 className="text-xl font-bold text-white mb-6 border-b border-white/10 pb-3">All Events</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
